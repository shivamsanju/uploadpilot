    ...
    return (session.email || session.name) ? (
        ...
        >
            <Menu.Target>
                <UnstyledButton>
                    <Group gap={7}>
                        <Avatar src={session.avatarUrl} alt={session.name[0]} radius="xl" size={30} />
                        {width > 768 && <Text fw={500} size="sm" lh={1} mr={3}>
            ...
            <Menu.Dropdown>
                <Menu.Item leftSection={<IconUser size={16} />} onClick={handleProfileClick}><Text size='sm'>Profile</Text></Menu.Item>
                <Menu.Item leftSection={<IconSun size={16} />} closeMenuOnClick={false}> <ThemeSwitcher /></Menu.Item>
                <Menu.Divider />
                <Menu.Item c="red" leftSection={<IconLogout size={16} />} onClick={handleSignOut}><Text size='sm'>Logout</Text></Menu.Item>
            </Menu.Dropdown>

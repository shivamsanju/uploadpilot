...
import { IconAlertTriangle, IconCirclePlusFilled, IconCube3dSphere } from '@tabler/icons-react';
...
export const BaseNode = (node: any) => {
    ...
    const { setOpenedNodeId, nodes, onConnectEnd, openBlocksModal, openedNodeId } = useCanvas();
        ...
        <>
            ...
            <Paper p="sm" withBorder className={`${classes.node} ${openedNodeId === node.id ? classes.nodeActive : ''}`} w={300} radius="sm" onDoubleClick={() => setOpenedNodeId(node.id)}>
                <Grid>
                    ...
                    <Grid.Col span={1}>
                        <Box mt="-10">
                            {node?.data?.isComplete === false && <IconAlertTriangle size={16} color="orange" />}
                        </Box>
                    </Grid.Col>
                </Grid>
                <Handle

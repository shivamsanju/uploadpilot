...
import { useDeleteProcessorMutation, useEnableDisableProcessorMutation, useGetProcessors } from '../../apis/processors';
import AddProcessorForm from './Add';
import { timeAgo } from '../../utils/datetime';
import { useViewportSize } from '@mantine/hooks';


const ProcessorList = ({ opened, setOpened }: { opened: boolean, setOpened: any }) => {
    ...
    const { mutateAsync: enableDisableProcessor, isPending: isEnabling } = useEnableDisableProcessorMutation();
    const navigate = useNavigate();

    const handleRemoveProcessor = useCallback(async (processorId: string) => {
        if (!workspaceId || workspaceId === '' || !processorId || processorId === '') {
            showNotification({
                color: 'red',
                title: 'Error',
    ...
    const handleEnableDisableProcessor = useCallback(async (processorId: string, enabled: boolean) => {
        if (!workspaceId || workspaceId === '' || !processorId || processorId === '') {
            showNotification({
                color: 'red',
                title: 'Error',
                ...
                <div>
                    <Text fz="sm" fw={500}>
                        {item.triggers && item.triggers.length > 0 ? (
                            <>
                                {item.triggers.slice(0, 5).map((trigger: any, index: number) => <Pill mr="xs" size="xs" key={index} variant="light">{trigger}</Pill>)}
                                {item.triggers.length > 5 && <Pill size="xs" color="blue">+{item.triggers.length - 5}</Pill>}
                            </>
                        ) : 'No Triggers'}
                    </Text>
                    <Text c="dimmed" fz="xs">
                    ...
                    >
                        ...
                        <Menu.Dropdown>
                            ...
                            <Menu.Item
                                ...
                                onClick={() => handleEnableDisableProcessor(item.id, !item?.enabled)}
                            >
    ...
    ], [handleRemoveProcessor, handleEnableDisableProcessor, handleViewEdit, width, navigate, workspaceId]);

    ...
    queryFn: () => {
      if (!workspaceId || !webhookId) {
        return Promise.reject(
          new Error("workspaceId and webhookId are required"),
        );
      }
      return axiosInstance
        .get(`/workspaces/${workspaceId}/webhooks`)
...
export const useCreateWebhookMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: ({
      ...
      return axiosInstance
        .post(`/workspaces/${workspaceId}/webhooks`, webhook)
        .then((res) => res.data);
    },
    onSuccess: () => {
...
export const useUpdateWebhookMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: ({
      ...
      return axiosInstance
        .put(`/workspaces/${workspaceId}/webhooks/${webhookId}`, webhook)
        .then((res) => res.data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["webhooks"] });
...
export const useEnableDisableWebhookMutation = () => {
  ...
  return useMutation({
    ...
    mutationFn: ({
      ...
      return axiosInstance
        .patch(`/workspaces/${workspaceId}/webhooks/${webhookId}`, { enabled })
        .then((res) => res.data);
    },
    onSuccess: (_, { enabled }) => {
      queryClient.invalidateQueries({ queryKey: ["webhooks"] });

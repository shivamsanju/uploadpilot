...
func (ur *uploaderRepo) FindAll(ctx context.Context, skip int64, limit int64, search string) ([]models.Uploader, int64, error) {
	...
	if err != nil {
		g.Log.Errorf("failed to count documents: %s", err.Error())
		return nil, 0, err
	}
	...
	if err != nil {
		g.Log.Errorf("failed to find uploaders: %s", err.Error())
		return nil, 0, err
	}
	...
	if err := cursor.All(ctx, &cb); err != nil {
		g.Log.Errorf("failed to decode uploaders: %s", err.Error())
		return nil, 0, err
	}

	g.Log.Infof("found %d uploaders (total records: %d)", len(cb), totalRecords)
...
func (ur *uploaderRepo) UpdateConfig(ctx context.Context, id string, updatedData *models.UploaderConfig, updatedBy string) error {
	...
	g.Log.Infof("Updating config: %+v", update)
	collection := g.Db.Database(g.DbName).Collection(ur.collectionName)
...
func (ur *uploaderRepo) GetDataStoreCreds(ctx context.Context, id string) (map[string]interface{}, error) {
	...
	g.Log.Infof("found Uploader: %+v", results[0])

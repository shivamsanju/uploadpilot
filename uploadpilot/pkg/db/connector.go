...
func (sr *storageConnectorRepo) FindAll(ctx context.Context, skip int64, limit int64, search string) ([]models.StorageConnector, int64, error) {
	...
	if err != nil {
		g.Log.Errorf("failed to find storage connectors: %s", err.Error())
		return nil, 0, err
	}
	...
	if err := cursor.All(ctx, &cb); err != nil {
		g.Log.Errorf("failed to decode storage connectors: %s", err.Error())
		return nil, 0, err
	}

	g.Log.Infof("found %d storage connectors (total records: %d)", len(cb), totalRecords)
